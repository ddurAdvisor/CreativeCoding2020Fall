import processing.core.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class fluid_dynamics extends PApplet {/**
<b>Fluid Dynamics</b>
<p>
...
public void addSource ( float[][] x, float[][] s, float dt )
{
  for ( int i=0; i<gridSize; i++ )
    for ( int j=0; j<gridSize; j++ )
      x[i][j] += s[i][j]*dt;
  ...
  for (y = 1; y <= n; y++ ) {
    for ( x = 1; x <= n; x++ ) {
      ...
      if ( showDensity ) {
        ...
        col = gradientColours[PApplet.parseInt(s)];
...
public void setForceArea(float[][] field, int x, int y, float s, float r) {
  ...
  for ( i = PApplet.parseInt(range(x-r,1,n)); i <= PApplet.parseInt(range(x+r,1,n)); i++ ) {
    dx = x - i;
    for ( j = PApplet.parseInt(range(y-r,1,n)); j <= PApplet.parseInt(range(y+r,1,n)); j++ ) {
      dy = y - j;
...
class Gradient {
  ...
  */
  ...
  public void addNode(float location, int col) {
  ...
  public void makeDefaultGradient(int defaultGradient) {
  ...
  public void makeRandomGradient(int numColours) {
    ...
    for ( int n=0; n < numColours; n++ ) {
      if ( n == 0 )
        ...
        locationMin = PApplet.parseFloat(n)/numColours;
        locationMax = PApplet.parseFloat(n+1)/numColours;
        location = random ( locationMin, locationMax );
      }

      r = PApplet.parseInt(random(2.5f)) * 128;
      g = PApplet.parseInt(random(2.5f)) * 128;
      b = PApplet.parseInt(random(2.5f)) * 128;
      
      addNode ( location, color(r,g,b) );
    }
  }
  
  public int getColour(float location) {
  ...
  public int[] makeArrayOfColours(int numColours) {
    
    float location, bandLocation, bandScale, bandDelta;
    int[] cols = new int[numColours];
  
    for ( int i=0; i < numColours; i++ ) {
      location = PApplet.parseFloat(i) / (numColours-1);
  ...
  static public void main(String args[]) {     PApplet.main(new String[] { "fluid_dynamics" });  }}